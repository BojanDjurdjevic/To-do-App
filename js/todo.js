const ToDo = function(rootElementAll, rootElementActive, rootElementCompl) {

    this.rootElementAll = rootElementAll
    this.rootElementActive = rootElementActive
    this.rootElementCompl = rootElementCompl
    ...
    function generateToDoItemView (toDoItem) {
        ...
        let dataElem = toDoItemRoot.getElementsByClassName("data")[0].innerHTML = 
        toDoItem.content;
        ...
        if(toDoItem.completed) {
            rootElementCompl.append(toDoItemRootCopy)
        } else {
        ...
        registerDeleteHendlers(toDoItemViewModel)

        registerClickHendlers(toDoItemViewModel)

    }

    function registerDeleteHendlers (toDoViewModel) {
    ...
    function registerClickHendlers(toDoItemViewModel) {
        for(let i = 0; i < toDoItemViewModel.views.length; i++) {
            ...
            toDoItemViewModel.views[i].onclick = function (e) {
                ...
                if(toDoItemViewModel.data.completed) {
                    ...
                    rootElementCompl.appendChild(toDoItemViewModel.views[1])
                } else {
